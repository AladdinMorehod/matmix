...
async function main() {
  ...
  const ceresitFile = tempFile("ceresit-ct17.db"); const ceresitDb = await fixture(ceresitFile, { definitions: [DATA.REUSABLE_DEFINITIONS.product_type, DATA.REUSABLE_DEFINITIONS.purpose, DATA.REUSABLE_DEFINITIONS.shelf_life] }); await addValue(ceresitDb, "MAT-000232", "product_type", "Универсальная грунтовка глубокого проникновения"); await addValue(ceresitDb, "MAT-000232", "purpose", "Укрепление впитывающих оснований, связывание пыли и снижение впитываемости"); await addValue(ceresitDb, "MAT-000232", "shelf_life", 24, "number"); const ceresitPlan = await inspectBatch(ceresitDb, { only: ["MAT-000232"] }); assert.equal(ceresitPlan.summary.valueConflict, 0); assert.equal(ceresitPlan.rows[0].specs.find(item => item.code === "product_type").status, "EXISTING_OK"); assert.equal(ceresitPlan.rows[0].specs.find(item => item.code === "purpose").status, "EXISTING_OK"); assert.equal(ceresitPlan.rows[0].specs.find(item => item.code === "shelf_life").status, "EXISTING_OK"); await ceresitDb.close(); pass("MAT-000232 authoritative CT 17 PRO values are EXISTING_OK");
