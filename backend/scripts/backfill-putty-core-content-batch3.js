...
const DATA = require("./data/putties-core-content-batch3");
const CONFIRM = "BACKFILL_PUTTY_CORE_BATCH3";
const PUTTY_MAT_ALLOWLIST = Object.freeze(Array.from({ length: 5 }, (_, i) => `MAT-${String(i + 61).padStart(6, "0")}`));
...
async function loadState(db, config) {
  ...
  if (!structure) throw new Error(`Structure guard failed: ${config.externalId}`);
...
function buildItem(code, proposal, def, existing, config) {
  ...
  if (status === "READY") {
    if (!def) {
      ...
      if (!creatable) return { code, status: "SCHEMA_BLOCKED", sourceStatus: "SCHEMA_BLOCKED", value: proposal.value, sources: proposal.sources || [], reason: "Reusable definition missing", definitionId: null, definitionType: null, currentValue: valueOf(existing), currentId: existing?.id };
    } else {
...
async function planProduct(db, config, preloadedDefinitions) {
  ...
  row.canonicalIdentity = { brand: config.brand, officialName: config.canonicalName || null, productType: config.core.product_type?.value || null, packageWeightKg: config.core.package_weight?.value || null, sourceKeys: config.sourceKeys };
  row.titleIssues = config.titleIssues || [];
...
async function inspectBatch(db, { only, data = DATA } = {}) {
  ...
  const summary = { total: rows.length, ready: rows.reduce((n,r) => n + (r.summary?.ready || 0), 0), partial: rows.filter(r => r.status === "PARTIAL").length, readyProducts: rows.filter(r => r.status === "READY").length, errors: rows.filter(r => r.status === "ERROR").length, willAdd: rows.reduce((n,r) => n + (r.summary?.willAdd || 0), 0), willFix: rows.reduce((n,r) => n + (r.summary?.willFix || 0), 0), existingOk: rows.reduce((n,r) => n + (r.summary?.existingOk || 0), 0), needsSource: rows.reduce((n,r) => n + (r.summary?.needsSource || 0), 0), absentByDesign: rows.reduce((n,r) => n + (r.summary?.absentByDesign || 0), 0), schemaBlocked: rows.reduce((n,r) => n + (r.summary?.schemaBlocked || 0), 0), logicalSlots: rows.reduce((n,r) => n + (r.summary?.logicalSlots || 0), 0), definitionsToCreate: definitionsToCreate(defs).length };
...
async function backupDatabase(dbPath, backupDir) { const dir = path.resolve(backupDir || path.join(path.dirname(dbPath), "backups")); fs.mkdirSync(dir, { recursive: true }); const target = path.join(dir, `matmix-before-putty-core-batch3-${new Date().toISOString().replace(/[:.]/g, "-")}.db`); fs.copyFileSync(path.resolve(dbPath), target); return target; }
