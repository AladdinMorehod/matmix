...
const DATA = require("./data/primers-descriptions");

const CONFIRM = "BACKFILL_PRIMER_DESCRIPTIONS";
...
function guard(product, config) {
  ...
  if (normalizeStructure(product.category) !== normalizeStructure(config.expectedCategory) || normalizeStructure(product.subcategory) !== normalizeStructure(config.expectedSubcategory)) throw new Error(`Category/subcategory guard failed: got ${product.category} / ${product.subcategory}`);
}
...
async function inspectBatch(db, { only, data = DATA } = {}) {
  ...
  for (const externalId of selected) {
    ...
    if (!config || !ALL_MATS.includes(externalId)) { rows.push({ externalId, status: "EXCLUDED_ALLOWLIST", reason: "MAT is outside exact primer description allowlist" }); continue; }
    try {
      validatePublicDescription(config.proposedDescription); const product = await db.get("SELECT * FROM products WHERE external_id=?", [externalId]); guard(product, config);
      const current = product.full_description ?? null; const proposed = config.proposedDescription;
      const status = !nonempty(current) ? "WILL_ADD" : current === proposed ? "EXISTING_OK" : "CONTENT_CONFLICT";
      rows.push({ externalId, productId: product.id, title: product.title, currentDescription: current, proposedDescription: proposed, status, sourceKeys: config.sourceKeys, notes: config.notes || [], immutableSnapshot: snapshot(product), characterCount: Array.from(proposed).length });
  ...
  return { mode: "dry-run", rows, sources: data.SOURCES, writableSurface: { products: DESCRIPTION_MUTABLE_FIELDS_EXACTLY, forbidden: IMMUTABLE_FIELDS }, summary: { total: rows.length, willAdd: rows.filter(r => r.status === "WILL_ADD").length, existingOk: rows.filter(r => r.status === "EXISTING_OK").length, contentConflict: rows.filter(r => r.status === "CONTENT_CONFLICT").length, errors: rows.filter(r => r.status === "ERROR").length, excludedAllowlist: rows.filter(r => r.status === "EXCLUDED_ALLOWLIST").length } };
}
...
async function backupDatabase(dbPath, backupDir) {
  ...
  const target = path.join(dir, `matmix-before-primer-descriptions-${Date.now()}.db`); fs.copyFileSync(path.resolve(dbPath), target); return target;
}
async function applyBatch(db, dbPath, options, data = DATA) {
  ...
  if (preflight.summary.errors || preflight.summary.excludedAllowlist || preflight.summary.contentConflict) throw new Error("Description apply blocked by guard/conflict");
  const targets = preflight.rows.filter(r => r.status === "WILL_ADD"); if (!targets.length) return { ...preflight, mode: "apply", writes: 0, backup: null };
  const backup = await backupDatabase(dbPath, options.backupDir); await db.run("BEGIN IMMEDIATE"); let writes = 0;
  try {
    const now = new Date().toISOString();
    for (const row of targets) { const result = await db.run("UPDATE products SET full_description=?,updated_at=? WHERE id=? AND external_id=? AND (full_description IS NULL OR TRIM(full_description)='')", [row.proposedDescription, now, row.productId, row.externalId]); if (result.changes !== 1) throw new Error(`Write guard failed: ${row.externalId}`); writes += 1; }
...
function renderReview(report) {
  const lines = ["# Грунтовки — descriptions review", "", "Изменяется только `products.full_description`; title, SEO, attributes, images, price и склад не изменяются.", "", `DESCRIPTION_MUTABLE_FIELDS_EXACTLY = ["full_description"]`, `CONFIRM = ${CONFIRM}`, "", "## Summary", "", "```json", JSON.stringify(report.summary, null, 2), "```", "", "| MAT | Status | Characters | Source keys |", "|---|---|---:|---|"];
  for (const row of report.rows) lines.push(`| ${row.externalId} | ${row.status} | ${row.characterCount || 0} | ${md((row.sourceKeys || []).join(", "))} |`);
  lines.push("", "## Proposed descriptions", "");
  for (const row of report.rows) lines.push(`### ${row.externalId}`, "", `- Title: ${md(row.title)}`, `- Status: **${row.status}**`, `- Source keys: ${md((row.sourceKeys || []).join(", "))}`, `- Notes: ${md((row.notes || []).join("; ") || "—")}`, "", row.proposedDescription || "—", "");
  lines.push("## Source registry", ""); for (const [key, source] of Object.entries(report.sources || {})) lines.push(`- **${key}**: ${source.url ? `[${source.title || key}](${source.url})` : source.title || key}`);
  return lines.join("\n") + "\n";
}
