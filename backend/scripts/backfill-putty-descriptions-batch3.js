...
const DATA = require("./data/putties-descriptions-batch3");

const CONFIRM = "BACKFILL_PUTTY_DESCRIPTIONS_BATCH3";
const ALL_MATS = DATA.TARGET_MATS;
const DESCRIPTION_MUTABLE_FIELDS_EXACTLY = Object.freeze(["full_description"]);
const PUBLIC_META_MARKERS = Object.freeze(["источник", "использованн", "официальн", "exact sku", "tds", "подтверждающ", "подтвержд", "проверенном", "числовое поле", "не преобразуется", "не указываем", "не приведены", "не указаны", "needs_source", "absent_by_design", "schema_blocked", "research", "review"]);
...
async function inspectBatch(db, { only, data = DATA } = {}) {
    ...
    for (const externalId of selected) {
        ...
        if (!ALL_MATS.includes(externalId)) { rows.push({ externalId, status: "EXCLUDED_ALLOWLIST", reasonForChange: "MAT отсутствует в явном allowlist Batch3." }); continue; }
...
async function backupDatabase(dbPath, backupDir) {
    ...
    const target = path.join(dir, `matmix-before-putty-descriptions-batch3-${new Date().toISOString().replace(/[:.]/g, "-")}.db`); fs.copyFileSync(path.resolve(dbPath), target); return target;
}
...
function renderReview(report) {
    const lines = ["# Putty full descriptions — Batch3 review", "", "Изменяется только `products.full_description`; production не используется.", "", `DESCRIPTION_MUTABLE_FIELDS_EXACTLY = [${DESCRIPTION_MUTABLE_FIELDS_EXACTLY.map(field => `\"${field}\"`).join(", ")}]`, "UNSUPPORTED_CLAIMS=0", "", "## Dry-run", "", "| MAT | Current title | Status | Characters | Source keys |", "|---|---|---|---:|---|"];
