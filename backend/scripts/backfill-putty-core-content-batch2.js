...
const DATA = require("./data/putties-core-content-batch2");
const CONFIRM = "BACKFILL_PUTTY_CORE_BATCH2";
const PUTTY_MAT_ALLOWLIST = Object.freeze(Array.from({ length: 14 }, (_, i) => `MAT-${String(i + 47).padStart(6, "0")}`));
...
async function backupDatabase(dbPath, backupDir) { const dir = path.resolve(backupDir || path.join(path.dirname(dbPath), "backups")); fs.mkdirSync(dir, { recursive: true }); const target = path.join(dir, `matmix-before-putty-core-batch2-${new Date().toISOString().replace(/[:.]/g, "-")}.db`); fs.copyFileSync(path.resolve(dbPath), target); return target; }
