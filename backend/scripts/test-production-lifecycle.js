...
(async () => {
    ...
    try {
        ...
        const env = { ...process.env, NODE_ENV: "test", PORT: String(port), SESSION_SECRET: "lifecycle-test-secret-123456789012345678901234", MATMIX_DB_PATH: dbPath, SESSION_DB_PATH: path.join(root, "sessions.db"), PRODUCT_UPLOADS_PATH: path.join(root, "uploads"), ORDER_ATTACHMENTS_PATH: path.join(root, "attachments"), BACKUP_ROOT_PATH: path.join(root, "backups"), APP_RUNTIME_LOCK_PATH: lock, PUBLIC_BASE_URL: base, SEO_ALLOW_INDEXING: "false" };
        await fs.promises.mkdir(env.ORDER_ATTACHMENTS_PATH, { recursive: true });
