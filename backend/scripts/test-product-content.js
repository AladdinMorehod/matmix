...
(async () => {
    ...
    try {
        ...
        result = await json(base, `/api/products/attribute-templates/${subcategory.id}`, "PUT", { templates: [{ definitionId: numberDefinition, section: "main", sortOrder: 0, isRequired: true, unitOverride: "мм" }, { definitionId: booleanDefinition, section: "regular", sortOrder: 0, isRequired: false }] }, admin); assert.strictEqual(result.response.status, 200); assert.strictEqual(result.data.templates.length, 2);
        assert.deepStrictEqual(result.data.templates.map(item => item.section), ["main", "regular"]);
        result = await json(base, `/api/products/attribute-templates/${subcategory.id}`, "GET", undefined, admin); assert.deepStrictEqual(result.data.templates.map(item => item.section), ["main", "regular"]);
        result = await json(base, `/api/products/attribute-templates/${subcategory.id}`, "PUT", { templates: [{ definitionId: numberDefinition, section: "main" }, { definitionId: numberDefinition, section: "regular" }] }, admin); assert.strictEqual(result.response.status, 409); assert.strictEqual(result.data.code, "DUPLICATE_TEMPLATE_DEFINITION");
        result = await json(base, `/api/products/attribute-templates/${subcategory.id}`, "PUT", { templates: [{ definitionId: numberDefinition, section: "unknown" }] }, admin); assert.strictEqual(result.response.status, 400); assert.strictEqual(result.data.code, "INVALID_TEMPLATE_SECTION");
        ...
        let checkDb = open(dbPath); const valuesBeforeTemplateSave = await all(checkDb, "SELECT * FROM product_attribute_values WHERE product_id=? ORDER BY id", [product.id]); await new Promise(resolve => checkDb.close(resolve));
        result = await json(base, `/api/products/attribute-templates/${subcategory.id}`, "PUT", { templates: [{ definitionId: booleanDefinition, section: "main", sortOrder: 0 }, { definitionId: numberDefinition, section: "main", sortOrder: 1 }] }, admin);
        assert.deepStrictEqual(result.data.templates.map(item => [item.code, item.section, item.sort_order]), [["is_frost_resistant", "main", 0], ["layer_thickness", "main", 1]]);
        result = await json(base, `/api/products/attribute-templates/${subcategory.id}`, "PUT", { templates: [{ definitionId: numberDefinition, section: "regular", sortOrder: 0 }, { definitionId: booleanDefinition, section: "main", sortOrder: 0 }] }, admin);
        assert.deepStrictEqual(result.data.templates.map(item => [item.code, item.section]), [["layer_thickness", "regular"], ["is_frost_resistant", "main"]]);
        result = await json(base, `/api/products/attribute-templates/${subcategory.id}`, "PUT", { templates: [{ definitionId: numberDefinition, section: "main", sortOrder: 0, isRequired: true, unitOverride: "мм" }, { definitionId: booleanDefinition, section: "regular", sortOrder: 0 }] }, admin);
        checkDb = open(dbPath); assert.deepStrictEqual(await all(checkDb, "SELECT * FROM product_attribute_values WHERE product_id=? ORDER BY id", [product.id]), valuesBeforeTemplateSave); await new Promise(resolve => checkDb.close(resolve));
        result = await json(base, `/api/products/${product.id}/content`, "PATCH", { removedDefinitionIds: [numberDefinition] }, admin); assert.strictEqual(result.response.status, 200);
        const afterMainValueRemoval = await json(base, `/api/products/${product.id}/content`, "GET", undefined, admin);
        assert(!afterMainValueRemoval.data.content.values.some(item => item.definitionId === numberDefinition && item.value !== ""));
        const templateAfterValueRemoval = await json(base, `/api/products/attribute-templates/${subcategory.id}`, "GET", undefined, admin);
        assert.strictEqual(templateAfterValueRemoval.data.templates.find(item => item.attribute_definition_id === numberDefinition).section, "main", "deleting a product value must retain template membership");
        result = await json(base, `/api/products/${product.id}/content`, "PATCH", { brand: "Knauf", attributes: [{ definitionId: numberDefinition, value: 25, unitOverride: "мм" }] }, admin); assert.strictEqual(result.response.status, 200);
