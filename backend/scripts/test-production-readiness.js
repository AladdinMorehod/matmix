...
(async () => {
    ...
    try {
        const dbPath = path.join(root, "runtime", "matmix.db"); const uploads = path.join(root, "runtime", "uploads"); const attachments = path.join(root, "runtime", "attachments"); const backups = path.join(root, "backups");
        await fs.promises.mkdir(path.dirname(dbPath), { recursive: true }); await fs.promises.mkdir(uploads, { recursive: true }); await fs.promises.mkdir(attachments, { recursive: true }); await fs.promises.mkdir(backups, { recursive: true });
        ...
        const paths = { dbPath, uploadsPath: uploads, attachmentsPath: attachments, backupRoot: backups, lockPath: env.APP_RUNTIME_LOCK_PATH, retentionCount: 3 };
        ...
        if (!valid.ready || !ready.ready || unsafe.ready || missingAttachments.ready || deployedAttachments.ready || temporaryAttachments.ready) throw new Error(JSON.stringify({ valid, ready, unsafe, missingAttachments, deployedAttachments, temporaryAttachments }));
        console.log(JSON.stringify({ success: true, productionConfig: valid.ready, operationalReadiness: ready.ready, backupVerified: ready.backup.verified, attachmentsAudited: ready.attachments.healthy, disk: ready.diskFreeMb, unsafeWebPathRejected: !unsafe.ready, missingAttachmentPathRejected: !missingAttachments.ready, deployedAttachmentPathRejected: !deployedAttachments.ready, temporaryAttachmentPathRejected: !temporaryAttachments.ready }));
