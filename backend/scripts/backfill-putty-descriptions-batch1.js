...
const DATA = require("./data/putties-descriptions-batch1");

const CONFIRM = "BACKFILL_PUTTY_DESCRIPTIONS_BATCH1";
...
async function inspectBatch(db, { only, data = DATA } = {}) {
    ...
    for (const externalId of selected) {
        ...
        if (!ALL_MATS.includes(externalId)) { rows.push({ externalId, status: "EXCLUDED_ALLOWLIST", reasonForChange: "MAT отсутствует в явном allowlist Batch1." }); continue; }
...
async function backupDatabase(dbPath, backupDir) {
    ...
    const target = path.join(dir, `matmix-before-putty-descriptions-batch1-${new Date().toISOString().replace(/[:.]/g, "-")}.db`); fs.copyFileSync(path.resolve(dbPath), target); return target;
}
...
function renderReview(report) {
    const lines = ["# Putty full descriptions — Batch1 review", "", "Изменяется только `products.full_description`; production не используется.", "", `DESCRIPTION_MUTABLE_FIELDS_EXACTLY = [${DESCRIPTION_MUTABLE_FIELDS_EXACTLY.map(field => `\"${field}\"`).join(", ")}]`, "UNSUPPORTED_CLAIMS=0", "", "## Dry-run", "", "| MAT | Current title | Status | Characters | Source keys |", "|---|---|---|---:|---|"];
