...
const DATA = require("./data/primers-core-batch2");

const CONFIRM = "BACKFILL_PRIMER_CORE_BATCH2";
...
async function inspectBatch(db, { only, data = DATA } = {}) {
  ...
  for (const externalId of selected) {
    ...
    if (!config || !ALL_MATS.includes(externalId)) rows.push({ externalId, status: "ERROR", error: `MAT is outside exact batch2 allowlist: ${externalId}` });
  ...
  return { mode: "dry-run", rows, summary, definitionsToCreate, sources: data.SOURCES, identityReview: data.IDENTITY_REVIEW || [], productionInspectionSql: data.PRODUCTION_INSPECTION_SQL || null, writableSurface: { products: PRODUCT_MUTABLE_FIELDS_EXACTLY, attributeValueCodes: DATA.CORE_ORDER, forbidden: ["title", "slug", "description", "short_description", "full_description", "seo_title", "seo_description", "price", "weight", "category", "subcategory", "image", "image_url", "stock_status", "any image table"] } };
}
...
async function backupDatabase(dbPath, backupDir) {
  ...
  const target = path.join(dir, `matmix-before-primer-core-batch2-${new Date().toISOString().replace(/[:.]/g, "-")}.db`); fs.copyFileSync(path.resolve(dbPath), target); return target;
}
...
function renderReview(report) {
  ...
  lines.push("", "## Final identity pass", "", "| MAT | status | current title | finding | missing evidence |", "|---|---|---|---|---|");
  for (const item of report.identityReview || []) lines.push(`| ${item.externalId} | ${item.status} | ${md(item.currentTitle)} | ${md(item.finding)} | ${md(item.missing)} |`);
  lines.push("", "## Read-only production inspection", "", "The following SQL is SELECT-only and must be run separately in the approved read-only production inspection process. This batch does not execute it:", "", "```sql", report.productionInspectionSql || "-- not configured", "```", "");
  return lines.join("\n") + "\n";
}
