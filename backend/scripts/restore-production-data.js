...
async function restore(backupPath, options = {}) {
    ...
    if (isInside(paths.uploadsPath, source) || isInside(paths.attachmentsPath, source) || source === path.resolve(paths.dbPath) || source === path.resolve(paths.uploadsPath) || source === path.resolve(paths.attachmentsPath)) throw new Error("Backup source conflicts with runtime targets.");
    if (!options.apply) return { success: true, dryRun: true, backupPath: source, references: verified.references, attachments: verified.attachments };
    ...
    try {
        ...
        await fs.promises.rm(oldDb, { force: true }); await fs.promises.rm(oldUploads, { recursive: true, force: true }); await fs.promises.rm(oldAttachments, { recursive: true, force: true });
        const report = { success: true, restoredAt: new Date().toISOString(), source, emergencyBackup: emergency.backupPath, references, attachments };
        ...
        if (attachmentsSwapped) await fs.promises.rm(paths.attachmentsPath, { recursive: true, force: true });
        if (fs.existsSync(oldAttachments)) await fs.promises.rename(oldAttachments, paths.attachmentsPath);
        ...
        await fs.promises.rm(stageDb, { force: true }); await fs.promises.rm(stageUploads, { recursive: true, force: true }); await fs.promises.rm(stageAttachments, { recursive: true, force: true }); throw error;
    }
}
