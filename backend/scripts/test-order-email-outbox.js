...
async function assertSchema(db) {
    assert.strictEqual(Number((await db.get("PRAGMA user_version")).user_version), 11);
...
async function assertMigration(root, sourceDatabase) {
    ...
    assert.strictEqual(repeated.fromVersion, 11);
}
...
async function main() {
    ...
    assert.strictEqual(freshMigration.toVersion, 11);
