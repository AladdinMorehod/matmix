...
async function main() {
    ...
    try {
        ...
        for (const [index, code] of codes.entries()) await db.run("INSERT INTO product_attribute_definitions(id,code,label,data_type,sort_order) VALUES(?,?,?,'text',?)", [index + 1, code, code, index === 4 ? 90 : index === 5 ? 20 : 10]);
        await replaceTemplates(2, [{ definitionId: 5, sortOrder: 5 }], db);
        ...
        assert.deepStrictEqual(content.values.map(item => item.code), [...codes.slice(0, 4), "alpha", "gamma", "beta"]);
        ...
        await assert.rejects(updateProductContent(1, { removedDefinitionIds: [3] }, db), error => error.code === "MAIN_ATTRIBUTE_REQUIRED");
        ...
        assert.deepStrictEqual((await getProductContent(1, db)).values.slice(4).map(item => item.code), ["alpha", "gamma", "beta"]);
        ...
        for (const id of [1, 2, 3, 4]) assert(!markup.includes(`data-remove-attribute="${id}"`));
        assert(markup.includes('name="attribute_2"'));
        assert.strictEqual((markup.match(/name="brand"/g) || []).length, 1);
        console.log("PASS attribute ordering, CRM/public parity, Test Brand A → B canonical edit, main edit and clear, legacy brand retained, row ID preservation, explicit removal, product isolation (in-memory SQLite)");
