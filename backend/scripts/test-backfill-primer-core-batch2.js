...
const { ALL_MATS, CONFIRM, DATA, PRODUCT_MUTABLE_FIELDS_EXACTLY, applyBatch, inspectBatch, parseArgs } = require("./backfill-primer-core-batch2");
...
function tempFile(name) { return path.join(fs.mkdtempSync(path.join(os.tmpdir(), "matmix-primer-batch2-")), name); }
...
async function main() {
  ...
  assert.deepEqual(PRODUCT_MUTABLE_FIELDS_EXACTLY, ["brand"]); assert.equal(ALL_MATS.length, 4); assert.equal(CONFIRM, "BACKFILL_PRIMER_CORE_BATCH2"); pass("exact allowlist and writable product surface");
  assert.throws(() => parseArgs(["--db", "x"]), /--only/); assert.throws(() => parseArgs(["--db", "x", "--only", ALL_MATS[0], "--apply", "--confirm", "WRONG"]), /BACKFILL_PRIMER_CORE_BATCH2/); pass("argument and apply confirmation guards");
  ...
  assert.equal(dry.summary.total, 4); assert.equal(dry.summary.errors, 0); assert.equal(dry.summary.titleGuardBlocked, 0); assert.ok(dry.summary.willAdd > 0); assert.ok(dry.summary.definitionsToCreate > 0); assert.equal(sha(emptyFile), before); pass("full dry-run is read-only and resolves missing definitions at runtime");
  const first = dry.rows.find(row => row.externalId === "MAT-000234"); assert.equal(first.specs.find(item => item.code === "package_weight").value, 10); assert.equal(first.specs.find(item => item.code === "package_volume").status, "NEEDS_SOURCE"); assert.equal(first.specs.find(item => item.code === "package_volume").value, null); pass("kg/l discrepancy never converts or backfills volume from title");
  ...
  const applyFile = tempFile("apply.db"); const applyDb = await fixture(applyFile); const applied = await applyBatch(applyDb, applyFile, { only: ALL_MATS, confirm: CONFIRM, backupDir: path.dirname(applyFile) }); assert.equal(applied.summary.errors, 0); assert.equal(applied.summary.titleGuardBlocked, 0); assert.equal((await applyDb.get("SELECT brand FROM products WHERE external_id=?", ["MAT-000234"])).brand, "LITOKOL"); const immutableAfter = await applyDb.get("SELECT full_description,seo_title,image_url,price,weight,stock_status,title,slug,category,subcategory FROM products WHERE external_id=?", ["MAT-000234"]); assert.equal(immutableAfter.full_description, "keep full"); assert.equal(immutableAfter.seo_title, "keep seo"); assert.equal(immutableAfter.image_url, "/keep.jpg"); assert.equal(immutableAfter.price, 100); assert.equal(immutableAfter.weight, 1); assert.equal(immutableAfter.stock_status, "in_stock"); assert.equal(immutableAfter.title, DATA.PRODUCTS.find(item => item.externalId === "MAT-000234").expectedTitle); assert.equal(immutableAfter.category, "Грунт / БетонКонтакт"); assert.equal(immutableAfter.subcategory, "Грунтовка"); pass("apply writes only brand/core targets and preserves content, price, weight, stock, image");
  ...
  const reuseFile = tempFile("reuse.db"); const reuseDb = await fixture(reuseFile, { definitions: [DATA.REUSABLE_DEFINITIONS.brand, DATA.REUSABLE_DEFINITIONS.product_type, DATA.REUSABLE_DEFINITIONS.base, DATA.REUSABLE_DEFINITIONS.purpose, DATA.REUSABLE_DEFINITIONS.package_weight, DATA.REUSABLE_DEFINITIONS.application_temperature, DATA.REUSABLE_DEFINITIONS.shelf_life, ...DATA.NEW_DEFINITIONS] }); const reusePlan = await inspectBatch(reuseDb, { only: ["MAT-000234"] }); assert.equal(reusePlan.summary.definitionsToCreate, 0); pass("existing compatible definitions are reused");
  await addValue(reuseDb, "MAT-000234", "brand", "LITOKOL"); const samePlan = await inspectBatch(reuseDb, { only: ["MAT-000234"] }); assert.equal(samePlan.rows[0].brand.attributeStatus, "EXISTING_OK"); pass("existing same value is EXISTING_OK");
  await reuseDb.run("UPDATE product_attribute_values SET value_text='Other' WHERE id=(SELECT v.id FROM product_attribute_values v JOIN products p ON p.id=v.product_id JOIN product_attribute_definitions d ON d.id=v.attribute_definition_id WHERE p.external_id='MAT-000234' AND d.code='brand')"); const conflictPlan = await inspectBatch(reuseDb, { only: ["MAT-000234"] }); assert.equal(conflictPlan.summary.brandConflict, 1); assert.equal(conflictPlan.rows[0].brand.status, "BRAND_CONFLICT"); await reuseDb.close(); pass("existing different brand attribute is BRAND_CONFLICT");

  const productBrandFile = tempFile("product-brand-conflict.db"); const productBrandDb = await fixture(productBrandFile, { definitions: [DATA.REUSABLE_DEFINITIONS.brand] }); await productBrandDb.run("UPDATE products SET brand='Other' WHERE external_id=?", ["MAT-000234"]); const productBrandPlan = await inspectBatch(productBrandDb, { only: ["MAT-000234"] }); assert.equal(productBrandPlan.summary.brandConflict, 1); assert.equal(productBrandPlan.rows[0].brand.productColumnStatus, "BRAND_CONFLICT"); await assert.rejects(() => applyBatch(productBrandDb, productBrandFile, { only: ["MAT-000234"], confirm: CONFIRM, backupDir: path.dirname(productBrandFile) }), /preflight guard/); await productBrandDb.close(); pass("existing different products.brand is BRAND_CONFLICT and blocks apply");

  const schemaFile = tempFile("schema.db"); const schemaDb = await fixture(schemaFile, { definitions: [DATA.REUSABLE_DEFINITIONS.brand, { code: "package_weight", ...DATA.REUSABLE_DEFINITIONS.package_weight, dataType: "text" }] }); const schemaPlan = await inspectBatch(schemaDb, { only: ["MAT-000234"] }); assert.equal(schemaPlan.rows[0].specs.find(item => item.code === "package_weight").status, "SCHEMA_BLOCKED"); await schemaDb.close(); pass("incompatible definition type is SCHEMA_BLOCKED");

  const titleFile = tempFile("title.db"); const titleDb = await fixture(titleFile); await titleDb.run("UPDATE products SET title='wrong' WHERE external_id=?", ["MAT-000234"]); const titlePlan = await inspectBatch(titleDb, { only: ["MAT-000234"] }); assert.equal(titlePlan.rows[0].status, "TITLE_GUARD_BLOCKED"); assert.equal(titlePlan.summary.titleGuardBlocked, 1); await assert.rejects(() => applyBatch(titleDb, titleFile, { only: ["MAT-000234"], confirm: CONFIRM, backupDir: path.dirname(titleFile) }), /preflight guard/); await titleDb.close(); pass("title guard blocks planning/apply without mutation");

  const categoryCaseFile = tempFile("category-case.db"); const categoryCaseDb = await fixture(categoryCaseFile); await categoryCaseDb.run("UPDATE products SET category=? WHERE external_id=?", ["Грунт / Бетонконтакт", "MAT-000234"]); const categoryCasePlan = await inspectBatch(categoryCaseDb, { only: ["MAT-000234"] }); assert.notEqual(categoryCasePlan.rows[0].status, "TITLE_GUARD_BLOCKED"); assert.equal(categoryCasePlan.summary.titleGuardBlocked, 0); await categoryCaseDb.close(); pass("category guard accepts case-only Unicode differences");
  const categoryWhitespaceFile = tempFile("category-whitespace.db"); const categoryWhitespaceDb = await fixture(categoryWhitespaceFile); await categoryWhitespaceDb.run("UPDATE products SET category=?,subcategory=? WHERE external_id=?", ["  грунт   /   БЕТОНКОНТАКТ  ", "  ГРУНТОВКА  ", "MAT-000234"]); const categoryWhitespacePlan = await inspectBatch(categoryWhitespaceDb, { only: ["MAT-000234"] }); assert.notEqual(categoryWhitespacePlan.rows[0].status, "TITLE_GUARD_BLOCKED"); assert.equal(categoryWhitespacePlan.summary.titleGuardBlocked, 0); await categoryWhitespaceDb.close(); pass("category guard collapses whitespace and ignores case");
  const wrongCategoryFile = tempFile("wrong-category.db"); const wrongCategoryDb = await fixture(wrongCategoryFile); await wrongCategoryDb.run("UPDATE products SET category=? WHERE external_id=?", ["Другая категория", "MAT-000234"]); const wrongCategoryPlan = await inspectBatch(wrongCategoryDb, { only: ["MAT-000234"] }); assert.equal(wrongCategoryPlan.rows[0].status, "TITLE_GUARD_BLOCKED"); assert.equal(wrongCategoryPlan.summary.titleGuardBlocked, 1); await wrongCategoryDb.close(); pass("real category mismatch remains blocked");
  const wrongSubcategoryFile = tempFile("wrong-subcategory.db"); const wrongSubcategoryDb = await fixture(wrongSubcategoryFile); await wrongSubcategoryDb.run("UPDATE products SET subcategory=? WHERE external_id=?", ["Другая подкатегория", "MAT-000234"]); const wrongSubcategoryPlan = await inspectBatch(wrongSubcategoryDb, { only: ["MAT-000234"] }); assert.equal(wrongSubcategoryPlan.rows[0].status, "TITLE_GUARD_BLOCKED"); assert.equal(wrongSubcategoryPlan.summary.titleGuardBlocked, 1); await wrongSubcategoryDb.close(); pass("subcategory mismatch remains blocked");
  const titleCaseFile = tempFile("title-case.db"); const titleCaseDb = await fixture(titleCaseFile); await titleCaseDb.run("UPDATE products SET title=? WHERE external_id=?", [DATA.PRODUCTS.find(item => item.externalId === "MAT-000234").expectedTitle.toLowerCase(), "MAT-000234"]); const titleCasePlan = await inspectBatch(titleCaseDb, { only: ["MAT-000234"] }); assert.equal(titleCasePlan.rows[0].status, "TITLE_GUARD_BLOCKED"); assert.equal(titleCasePlan.summary.titleGuardBlocked, 1); await titleCaseDb.close(); pass("exact title guard remains case-sensitive");

  const rollbackFile = tempFile("rollback.db"); const rollbackDb = await fixture(rollbackFile); const rollbackBefore = sha(rollbackFile); const failing = { ...rollbackDb, run(sql, params) { if (sql.startsWith("INSERT INTO product_attribute_values") && params?.[0] === 1) return Promise.reject(new Error("injected rollback")); return rollbackDb.run(sql, params); } }; await assert.rejects(() => applyBatch(failing, rollbackFile, { only: ["MAT-000234"], confirm: CONFIRM, backupDir: path.dirname(rollbackFile) }), /injected rollback/); assert.equal(sha(rollbackFile), rollbackBefore); assert.equal(await scalar(rollbackDb, "SELECT COUNT(*) n FROM product_attribute_values"), 0); await rollbackDb.close(); pass("backup precedes mutation and rollback restores definitions and values");
