...
async function main() {
  ...
  assert.equal(dry.summary.total, 15); assert.equal(dry.summary.errors, 0); assert.equal(dry.summary.titleGuardBlocked, 0); assert.equal(dry.summary.definitionsToCreate, 3); assert.equal(dry.summary.imageCleanupEligible, 15); assert.equal(dry.summary.brandConflict, 0); assert.equal(sha(dryFile), before); assert.equal(PRODUCT_MUTABLE_FIELDS_EXACTLY.join(","), "brand,image_url"); assert.equal(item(row(dry, "MAT-000075"), "package_weight").status, "WILL_ADD"); assert.equal(item(row(dry, "MAT-000077"), "layer_thickness").status, "NEEDS_SOURCE"); assert.equal(item(row(dry, "MAT-000090"), "package_weight").value, 25); pass("read-only dry-run preserves DB and keeps unresolved source fields"); await dryDb.close();
