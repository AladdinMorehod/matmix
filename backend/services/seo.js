...
function productPage(config, pageData) {
    ...
    const body = `<nav class="catalog-breadcrumbs product-page-breadcrumbs" aria-label="Хлебные крошки">${crumbs.map((item, i) => `${i ? "<span>/</span>" : ""}${i === crumbs.length - 1 ? `<span class="current" aria-current="page">${escapeHtml(item.name)}</span>` : `<a href="${escapeHtml(item.path)}">${escapeHtml(item.name)}</a>`}`).join("")}</nav>
        ...
        <dialog class="product-page-one-click-dialog" data-one-click-dialog aria-labelledby="oneClickTitle"><form method="dialog"><button class="product-page-dialog-close" value="cancel" aria-label="Закрыть">×</button><h2 id="oneClickTitle">Заказать в 1 клик</h2><p>Оставьте заявку через обычную корзину — менеджер свяжется с вами для подтверждения деталей.</p><div><button value="cancel">Продолжить покупки</button><a href="/catalog">Перейти в каталог</a></div></form></dialog><script src="/js/product-page.js" defer></script>`;
