...
const PUBLIC_ASSET_VERSION = "20260916-product-page-2";
...
function seoConfig(env = process.env) {
    ...
    return { indexing, baseUrl, siteName: SITE_NAME, defaultOgImage: String(env.DEFAULT_OG_IMAGE || "/img/logo-burgundy.png"), yandexMetrikaId: /^\d+$/.test(String(env.MATMIX_YANDEX_METRIKA_ID || "").trim()) ? String(env.MATMIX_YANDEX_METRIKA_ID).trim() : "" };
}
...
function productPage(config, pageData) {
    ...
    const pricePerKg = hasPrice && Number(product.weight) > 0 ? `<p class="product-page-unit-price">${escapeHtml(formatMoneyValue(Number(product.price) / Number(product.weight)))} ₽/кг</p>` : "";
    const dataAttributes = `data-product-id="${Number(product.id)}" data-external-id="${escapeHtml(product.external_id)}" data-title="${escapeHtml(product.title)}" data-price="${Number(product.price) || 0}" data-weight="${Number(product.weight) || 0}" data-unit="${escapeHtml(product.unit || "шт")}"`;
    ...
    const body = `<nav class="catalog-breadcrumbs product-page-breadcrumbs" aria-label="Хлебные крошки">${crumbs.map((item, i) => `${i ? "<span>/</span>" : ""}${i === crumbs.length - 1 ? `<span class="current" aria-current="page">${escapeHtml(item.name)}</span>` : `<a href="${escapeHtml(item.path)}">${escapeHtml(item.name)}</a>`}`).join("")}</nav>
        <article class="product-page" ${dataAttributes}>
            <section class="product-page-hero">
                ...
                <div class="product-page-summary"><h1>${escapeHtml(product.title)}</h1><div class="product-page-meta">${product.brand ? `${escapeHtml(product.brand)} · ` : ""}Артикул ${escapeHtml(product.external_id)}</div>${lead ? `<p class="product-page-lead">${escapeHtml(lead)}</p>` : ""}<div class="product-page-price"><strong>${hasPrice ? `${escapeHtml(product.price)} ₽` : "Цена по запросу"}</strong>${hasPrice && product.unit ? `<span>/ ${escapeHtml(product.unit)}</span>` : ""}${pricePerKg}</div><p class="product-page-supply">Срок и стоимость доставки рассчитаем при оформлении</p><div class="product-page-buy"><div class="product-page-quantity" aria-label="Количество"><button type="button" data-quantity-minus aria-label="Уменьшить количество">−</button><input data-quantity type="number" min="1" max="999" step="1" value="1" aria-label="Количество товара"><button type="button" data-quantity-plus aria-label="Увеличить количество">+</button></div><button class="add-btn" type="button" data-add-product ${dataAttributes}>В корзину</button><button class="product-page-one-click" type="button" data-one-click>Заказать в 1 клик</button></div><span class="product-page-cart-badge" data-cart-count aria-live="polite"></span><div class="product-page-benefits" aria-label="Преимущества"><span class="product-page-benefit"><strong><span aria-hidden="true">🚚</span> Доставка по Москве и МО</strong><small>Сегодня / Завтра / Срочно</small></span><span class="product-page-benefit"><strong><span aria-hidden="true">🔎</span> Удобный поиск</strong><small>По всему каталогу</small></span><a class="product-page-benefit product-page-upload-cta" href="#uploadRequestForm" data-upload-request-cta><strong><span aria-hidden="true">📋</span> Заказ по списку</strong><small>Загрузите список — соберём заказ</small></a></div></div>
            </section>
            ${contentNavigation}
...
function categoryPage(config, node, parent, children, products, pageNumber, totalPages) {
    ...
    const productHtml = `<section><h2>Товары</h2><ul>${products.map(product => `<li><a href="${productPath(product)}">${escapeHtml(product.title)} — ${escapeHtml(product.external_id)}</a>${Number(product.price) > 0 ? `, ${escapeHtml(formatMoneyValue(product.price))} ₽` : ", цена по запросу"}</li>`).join("")}</ul></section>`;
