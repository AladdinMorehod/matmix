...
async function getOrderingContext(product, database) {
    ...
    const templates = structure ? await database.all("SELECT attribute_definition_id,sort_order FROM product_attribute_templates WHERE structure_id=?", [structure.id]) : [];
    return {
        ...
        templates: templates.map(row => ({ definitionId: row.attribute_definition_id, sortOrder: row.sort_order }))
    };
}
