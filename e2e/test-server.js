...
prepare().then(() => {
    const testSpecificEnv = process.env.MATMIX_E2E_YANDEX_METRIKA_ID ? { MATMIX_YANDEX_METRIKA_ID: process.env.MATMIX_E2E_YANDEX_METRIKA_ID } : {};
    child = spawn(process.execPath, [path.join(root, "backend", "server.js")], { cwd: root, windowsHide: true, stdio: ["inherit", "inherit", "inherit", "ipc"], env: { ...process.env, ...testSpecificEnv, NODE_ENV: "test", PORT: "4173", SESSION_SECRET: "e2e-only-secret-not-for-production-1234567890", MATMIX_DB_PATH: dbPath, SESSION_DB_PATH: path.join(runtime, "sessions.db"), PRODUCT_UPLOADS_PATH: path.join(runtime, "uploads"), ORDER_ATTACHMENTS_PATH: orderAttachmentsPath, CATALOG_IMPORT_ARCHIVE_PATH: path.join(runtime, "catalog-imports"), BACKUP_ROOT_PATH: path.join(runtime, "backups"), APP_RUNTIME_LOCK_PATH: path.join(runtime, "runtime.lock"), PUBLIC_BASE_URL: "http://127.0.0.1:4173", SEO_ALLOW_INDEXING: "true", LOGIN_RATE_MAX: "10", LOGIN_RATE_WINDOW_MS: "60000", FILE_REQUEST_RATE_MAX: "1000" } });
